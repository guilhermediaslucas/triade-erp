...
export class SqlPerfilRepository implements PerfilRepository {
  ...
  async listar(schema: string): Promise<Perfil[]> {
    ...
    const perfis = await this.ds.query(
      `SELECT id, nome, descricao, ativo, criado_em FROM "${s}".perfil ORDER BY nome`,
    );
  ...
  async buscarPorId(schema: string, id: string): Promise<Perfil | null> {
    const s = validarSchema(schema);
    const p = (await this.ds.query(`SELECT id, nome, descricao, ativo, criado_em FROM "${s}".perfil WHERE id = $1`, [id]))[0];
    if (!p) return null;
    const caps = await this.ds.query(`SELECT capability FROM "${s}".perfil_capability WHERE perfil_id = $1`, [id]);
    return { id: p.id, nome: p.nome, descricao: p.descricao ?? '', ativo: p.ativo !== false, criadoEm: new Date(p.criado_em), capabilities: caps.map((c: any) => c.capability) };
  }

  async criar(schema: string, nome: string, descricao: string, ativo: boolean, capabilities: string[]): Promise<Perfil> {
    const s = validarSchema(schema);
    const id = randomUUID();
    await this.ds.query(`INSERT INTO "${s}".perfil (id, nome, descricao, ativo) VALUES ($1, $2, $3, $4)`, [id, nome, descricao, ativo]);
    await this.gravarCaps(s, id, capabilities);
    return { id, nome, descricao, ativo, capabilities, criadoEm: new Date() };
  }

  async atualizar(schema: string, id: string, nome: string, descricao: string, ativo: boolean, capabilities: string[]): Promise<void> {
    const s = validarSchema(schema);
    await this.ds.query(`UPDATE "${s}".perfil SET nome = $2, descricao = $3, ativo = $4 WHERE id = $1`, [id, nome, descricao, ativo]);

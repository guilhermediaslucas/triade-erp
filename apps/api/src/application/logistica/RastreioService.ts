...
import type { RotaPublica, RotaRepository } from '../../domain/logistica/Rota.js';
...
export class RastreioService {
  constructor(private readonly repo: RastreioRepository, private readonly pedidos: PedidoRepository, private readonly rota: RotaRepository) {}
  ...
  // ===== ROTA do motoboy AVULSO (freelancer, sem login): um link p/ todas as paradas dele =====
  // Gera/retorna o token de rota do motoboy (link `/rota/<token>`).
  async gerarLinkRota(schema: string, motoboyId: any): Promise<string> {
    const novo = schema.replace(/^t_/, '') + '.' + randomBytes(8).toString('hex');
    const tok = await this.rota.garantirRotaToken(schema, String(motoboyId ?? ''), novo);
    if (!tok) throw new ErroAplicacao('rota.motoboy_invalido', 400);
    return tok;
  }

  // Visão pública da rota: paradas em ordem, com status + posição + ETA da que está em rota.
  async rotaPublica(schema: string, token: any): Promise<RotaPublica | null> {
    const mb = await this.rota.motoboyPorRotaToken(schema, String(token ?? ''));
    if (!mb) return null;
    const paradas = await this.rota.paradasPublicas(schema, mb.id);
    for (const p of paradas) {
      if (p.posicao && (p.status === 'a_caminho' || p.status === 'chegou')) {
        p.eta = await this.calcEta('rota:' + String(token) + ':' + p.pedidoId, p.posicao.lat, p.posicao.lng, p.enderecoEntrega);
      }
    }
    return { motoboyNome: mb.nome, paradas };
  }

  // Confirma que o pedido pertence ao motoboy do token de rota.
  private async paradaDaRota(schema: string, token: any, pedidoId: string) {
    const mb = await this.rota.motoboyPorRotaToken(schema, String(token ?? ''));
    if (!mb) throw new ErroAplicacao('rota.nao_encontrada', 404);
    const d = await this.repo.dono(schema, String(pedidoId));
    if (!d) throw new ErroAplicacao('pedido.nao_encontrado', 404);
    if (d.motoboyId !== mb.id) throw new ErroAplicacao('entrega.nao_sua', 403);
    return d;
  }
  ...
  async rotaStatus(schema: string, token: any, pedidoId: string, status: any, recebidoPor: any, codigoConfirmacao?: any): Promise<void> {
    ...
    await this.repo.definirStatus(schema, pedidoId, status as StatusEntrega, rt);
  }

  async rotaPosicao(schema: string, token: any, pedidoId: string, lat: any, lng: any): Promise<void> {
    const la = Number(lat), ln = Number(lng);
    if (!Number.isFinite(la) || !Number.isFinite(ln)) throw new ErroAplicacao('entrega.posicao_invalida', 400);
    const d = await this.paradaDaRota(schema, token, pedidoId);
    if (d.status !== 'a_caminho' && d.status !== 'chegou') return;

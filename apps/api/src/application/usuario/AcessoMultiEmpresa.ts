...
export class AcessoMultiEmpresa {
  ...
  async situacao(emailEntrada: string): Promise<AcessoEmpresaInfo[]> {
    const email = String(emailEntrada ?? '').trim().toLowerCase();
    const out: AcessoEmpresaInfo[] = [];
    for (const e of await this.empresas.listarTodas()) {
      ...
      out.push({ codigo: e.codigo, fantasia: e.fantasia, temAcesso: !!(u && u.ativo), existe: !!u, perfilNome });
    }
    return out;
  }

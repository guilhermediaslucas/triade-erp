...
export function rotasPedidos(deps: Dependencias): Router {
  ...
  r.patch('/pedidos/:id/status', aut, az('comercial.pedido.gerenciar'), async (req, res: Response) => {
    try { const b = req.body ?? {}; await deps.pedidosService.mudarStatus(sch(req), req.params.id!, b.status, { formaEnvio: b.formaEnvio, formaEnvioDetalhe: b.formaEnvioDetalhe, entregueEm: b.entregueEm, motoboyId: b.motoboyId }, req.usuario?.nome ?? null); res.json({ ok: true }); } catch (e) { tratarErro(res, e); }
  });

...
// Modal de anexos de um título financeiro (NF, conta de energia, etc.) — arquivos no R2.
export function AnexosTitulo({ tituloId, numero, podeGerenciar, onFechar }: {
  ...
  function carregar() { api.get<Anexo[]>(`/financeiro/titulos/${tituloId}/anexos`, token!).then(setItens).catch((e) => setErro((e as ErroApi).chaveI18n)); }
  useEffect(() => {
    carregar();
  ...
  function escolher(e: ChangeEvent<HTMLInputElement>) {
    ...
    leitor.onload = async () => {
      ...
      try {
        await api.post(`/financeiro/titulos/${tituloId}/anexos`, { nomeArquivo: arq.name, tipo: arq.type, conteudoBase64: String(leitor.result) }, token!);
        carregar();
  ...
  async function remover(a: Anexo) {
    ...
    try { await api.del(`/financeiro/anexos/${a.id}`, token!); carregar(); } catch (e) { setErro((e as ErroApi).chaveI18n); }
  }

  return (
    <div className="modal-fundo"><div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 520 }}>
      <h2><Ic name="i-clip" /> {t('anexo.titulo')} — {numero}</h2>

...
export function RelCategorias() {
  ...
  return (
    <div>
      <h1 className="page-titulo">{t('rel.categorias')}</h1>
      <div className="rel-filtro">
        ...
        {linhas.length > 0 && <button className="btn-ghost" onClick={() => baixarCsv('vendas_categoria_' + de + '_' + ate, [t('produtos.categoria'), t('rel.qtd'), t('rel.total')], linhas.map((l) => [l.categoria, l.quantidade, l.total]))}>{t('rel.exportar')}</button>}
      </div>

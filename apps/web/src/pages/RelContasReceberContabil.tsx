...
interface Titulo {
  ...
  status: 'aberto' | 'pago'; categoriaFinanceiraNome: string | null;
}
...
export function RelContasReceberContabil() {
  ...
  useEffect(() => { api.get<Titulo[]>('/financeiro/receber', token!).then(setItens).catch((e) => setErro((e as ErroApi).chaveI18n)); /* eslint-disable-next-line */ }, []);

  const filtrados = useMemo(() => itens.filter((x) => {
    const e = emissaoDe(x);
    return (!de || e >= de) && (!ate || e <= ate);
  }), [itens, de, ate]);
  const aReceber = filtrados.filter((x) => x.status === 'aberto').reduce((a, x) => a + x.valor, 0);
  const recebido = filtrados.filter((x) => x.status === 'pago').reduce((a, x) => a + x.valor, 0);

  function exportar(fmt: 'csv' | 'xlsx') {
    const cab = [t('fin.numero'), t('fin.descricao'), t('relcp.categoria'), t('relcr.cliente'), t('fin.emissao'), t('fin.vencimento'), t('fin.valor'), t('fin.situacao'), t('relcp.anexos')];
    const dados = filtrados.map((x) => [x.numero, x.descricao, x.categoriaFinanceiraNome ?? '', x.pessoaNome ?? '', fmtData(emissaoDe(x)), fmtData(x.vencimento), x.valor, t('fin.' + x.status), x.anexosCount]);
    if (fmt === 'xlsx') baixarExcel('contas_receber_' + de + '_' + ate, cab, dados, { periodo: rotuloPeriodo(de, ate) });
    else baixarCsv('contas_receber_' + de + '_' + ate, cab, dados);
  }

  if (erro) return <div className="alerta-erro">{t(erro)}</div>;
  return (
    <div>
      <CabecalhoRelatorio titulo={t('relcr.titulo')} />
      <div className="crumb">{t('relcr.crumb')}</div><h1 className="page-titulo">{t('relcr.titulo')}</h1><p className="muted page-sub">{t('relcr.sub_emissao')}</p>
      ...
      <div className="kpi-row" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
        <div className="card kpi-mock"><div className="kpi-ic tint-gr"><Ic name="i-dollar" className="sm" /></div><div className="kpi-body"><div className="kpi-lbl">{t('relcr.total')}</div><div className="kpi-val">{moeda(aReceber)}</div></div></div>
        <div className="card kpi-mock"><div className="kpi-ic tint-bl"><Ic name="i-check" className="sm" /></div><div className="kpi-body"><div className="kpi-lbl">{t('relcr.recebido')}</div><div className="kpi-val" style={{ color: '#166534' }}>{moeda(recebido)}</div></div></div>
      ...
      <div className="card pad0">
        <table className="tabela tabela-1linha">
          <thead><tr><th>{t('fin.numero')}</th><th>{t('fin.descricao')}</th><th>{t('relcp.categoria')}</th><th>{t('relcr.cliente')}</th><th>{t('fin.emissao')}</th><th>{t('fin.vencimento')}</th><th style={{ textAlign: 'right' }}>{t('fin.valor')}</th><th style={{ textAlign: 'center' }}>{t('fin.situacao')}</th><th style={{ textAlign: 'center' }}>{t('relcp.anexos')}</th></tr></thead>
          <tbody>
            ...
            {filtrados.map((x) => (
              <tr key={x.id}>
                ...
                <td>{x.pessoaNome ?? '—'}</td><td>{fmtData(emissaoDe(x))}</td><td style={{ color: 'var(--muted)' }}>{fmtData(x.vencimento)}</td>

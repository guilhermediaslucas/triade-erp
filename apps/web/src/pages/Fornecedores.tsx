...
interface Fornecedor { id: string; nome: string; fantasia: string | null; documento: string; email: string | null; telefone: string | null; ativo: boolean; }
const vazio = (): Fornecedor => ({ id: '', nome: '', fantasia: '', documento: '', email: '', telefone: '', ativo: true });
  ...
  return (
    <div>
      ...
      <div className="card pad0"><table className="tabela">
        <thead><tr><th>{t('pessoa.nome')}</th><th>{t('pessoa.documento')}</th><th>{t('pessoa.email')}</th><th>{t('pessoa.telefone')}</th><th>{t('usuarios.situacao')}</th><th>{t('usuarios.acoes')}</th></tr></thead>
        <tbody>
          ...
          {itens.map((f) => (
            <tr key={f.id} className={f.ativo ? '' : 'linha-inativa'}>
              ...
              <td>{f.documento}</td><td>{f.email ?? '—'}</td><td>{f.telefone ?? '—'}</td>
              <td><span className={f.ativo ? 'pill-ok' : 'pill-off'}>{f.ativo ? t('usuarios.ativo') : t('usuarios.inativo')}</span></td>
              <td className="acoes">{pode && <>
                <button className="btn-link" onClick={() => setEdit({ ...f, fantasia: f.fantasia ?? '', email: f.email ?? '', telefone: f.telefone ?? '' })}>{t('common.editar')}</button>
...
function ModalForn({ f, onFechar, onSalvo }: { f: Fornecedor; onFechar: () => void; onSalvo: () => void; }) {
  ...
  const set = (c: keyof Fornecedor, val: any) => setV({ ...v, [c]: val });
  async function salvar() {
    setErro(null); setSalv(true);
    const corpo = { nome: v.nome, fantasia: v.fantasia, documento: v.documento, email: v.email, telefone: v.telefone };
  ...
  return (
    <div className="modal-fundo" onClick={onFechar}><div className="modal" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="cores-grid">
        <label className="campo">{t('pessoa.documento')}<input value={v.documento} onChange={(e) => set('documento', e.target.value)} /></label>

    ...
    (async () => {
      ...
      try {
        if (temCapability('financeiro.receber.listar')) {
          ...
          if (venc > 0) out.push({ chave: 'sino.titulos_vencidos', icone: '💸', tint: 'tint-rd', qtd: venc, to: '/financeiro/aging-receber' });
          ...
          if (pend > 0) out.push({ chave: 'sino.pendencia_baixa', icone: '🧾', tint: 'tint-or', qtd: pend, to: '/financeiro/receber' });
        }
        if (temCapability('relatorios.ver')) {
          ...
          if (vencendo > 0) out.push({ chave: 'sino.lotes_vencendo', icone: '⏳', tint: 'tint-or', qtd: vencendo, to: '/relatorios/validade' });
        }
        if (temCapability('estoque.saldo.ver')) {
          ...
          if (baixo > 0) out.push({ chave: 'sino.estoque_baixo', icone: '📦', tint: 'tint-bl', qtd: baixo, to: '/estoque/posicao' });
        }
      } catch { /* silencioso */ }
  ...
  return (
    <div>
      ...
      {!carregando && grupos.length === 0 && (
        <div className="card" style={{ textAlign: 'center', padding: 40 }}><div style={{ fontSize: 40 }}>✅</div><div className="muted" style={{ marginTop: 8 }}>{t('notif.vazio')}</div></div>
      )}
          ...
          <div className="alerts">
            {grupos.map((g) => (
              <Link key={g.chave} to={g.to} className="alert">
                <div className="top"><div className={'kpi-ic sm ' + g.tint}>{g.icone}</div><div className="big">{g.qtd}</div></div>

...
interface Ativa { pedidoId: string; numero: number; clienteNome: string | null; motoboy: string | null; status: string; rastreioToken: string | null; enderecoEntrega: string | null; posicao: Pos | null; }
    ...
    <div>
      ...
      {ativas.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 16, alignItems: 'start' }}>
          ...
          <div className="card">
            {selecionada
              ? <MapaEntrega lat={selecionada.posicao?.lat ?? null} lng={selecionada.posicao?.lng ?? null} altura={420} />
              : <div className="muted">{t('rastreio.selecione')}</div>}
            {selecionada?.posicao && <div className="muted" style={{ fontSize: 12, marginTop: 8 }}>{t('rastreio.atualizado_em')}: {new Date(selecionada.posicao.criadoEm).toLocaleString('pt-BR')}</div>}
          </div>
        </div>
      )}
    </div>
